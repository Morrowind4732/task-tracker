...
// Dice that should remain visible after landing, such as lobby initiative rolls.
let heldDice = [];
...
function screenToRestPlanePoint(clientX, clientY) {
  ensureScene();
  const x = Number(clientX);
  const y = Number(clientY);
  if (!Number.isFinite(x) || !Number.isFinite(y) || !camera) return null;
  ...
  // Use a camera-facing plane through the normal centered resting point.
  // The old horizontal table-plane projection cannot represent UI targets
  // high on the lobby screen; for those points the ray misses the plane and
  // the die falls back to screen center. A camera-facing plane lets any
  // commander-card pixel map to a visible 3D point.
  const viewDir = new THREE.Vector3();
  camera.getWorldDirection(viewDir).normalize();
  const anchor = new THREE.Vector3(0, restY, 0);
  ...
  // Extremely defensive fallback: still honor the supplied screen point.
  return new THREE.Vector3(ndc.x, ndc.y, 0.5).unproject(camera);
}
...
// ============================================================================
function createDie(type, d10OverlayNums = null, options = {}) {
  const preserveExisting = Boolean(options?.preserveExisting);
  if (!preserveExisting && die) scene.remove(die);
  if (!blobShadow) blobShadow = createBlobShadow();
  blobShadow.visible = !Boolean(options?.hideShadow);
  currentDie = type;
...
function clearActiveDice() {
  if (!scene) return;
  const removed = new Set();
  const removeMesh = (mesh) => {
    if (!mesh || removed.has(mesh)) return;
    scene.remove(mesh);
    removed.add(mesh);
  };
  removeMesh(die);
  die = null;
  for (const m of heldDice) removeMesh(m);
  heldDice = [];
  if (d100Dice.length) {
    for (const m of d100Dice) removeMesh(m);
    d100Dice = [];
  }
...
function getDieWorldCenterMeshGeneric(mesh) {
  const p = new THREE.Vector3();
  mesh.getWorldPosition(p);
  return p;
}

function getFaceCentersWorldForMesh(mesh, type) {
  const out = [];
  if (!mesh?.geometry) return out;
  const g = mesh.geometry;
  const pos = g.attributes.position;
  mesh.updateMatrixWorld(true);
  const dieCenterW = getDieWorldCenterMeshGeneric(mesh);
  const order = numbering[type] || [];
  for (let i = 0; i < pos.count; i += 3) {
    const a = new THREE.Vector3()
      .fromBufferAttribute(pos, i)
      .applyMatrix4(mesh.matrixWorld);
...
function getFacingNumberD6ForMesh(mesh) {
  const dieCenterW = getDieWorldCenterMeshGeneric(mesh);
  const dieToCam = new THREE.Vector3()
    .subVectors(camera.position, dieCenterW)
    .normalize();
...
function getFacingNumberForMesh(mesh, type) {
  if (type === 'd6') return getFacingNumberD6ForMesh(mesh);
  const dieCenterW = getDieWorldCenterMeshGeneric(mesh);
  const dieToCam = new THREE.Vector3()
    .subVectors(camera.position, dieCenterW)
    .normalize();
  const faces = getFaceCentersWorldForMesh(mesh, type);
  let best = null;
  let bestDot = -Infinity;
  for (const f of faces) {
    const d = f.dirFromDie.dot(dieToCam);
...
function getFaceAxesWorldD6ForMesh(mesh, number) {
  const local = {
    1: { n: new THREE.Vector3(0, 0, 1), up: new THREE.Vector3(0, 1, 0) },
    6: { n: new THREE.Vector3(0, 0, -1), up: new THREE.Vector3(0, 1, 0) },
    2: { n: new THREE.Vector3(0, 1, 0), up: new THREE.Vector3(0, 0, -1) },
    5: { n: new THREE.Vector3(0, -1, 0), up: new THREE.Vector3(0, 0, 1) },
    3: { n: new THREE.Vector3(1, 0, 0), up: new THREE.Vector3(0, 1, 0) },
    4: { n: new THREE.Vector3(-1, 0, 0), up: new THREE.Vector3(0, 1, 0) }
  }[number];
  if (!local) return null;
  const q = new THREE.Quaternion();
  mesh.getWorldQuaternion(q);
...
function getFaceTextureAxesWorldForMesh(mesh, type, number) {
  if (!mesh?.geometry) return null;
  if (type === 'd6') return getFaceAxesWorldD6ForMesh(mesh, number);
  ...
  for (let i = 0; i < pos.count; i += 3) {
    ...
    const e2 = new THREE.Vector3().subVectors(p3, p1);
    const du1 = uv2.x - uv1.x;
    const dv1 = uv2.y - uv1.y;
    const du2 = uv3.x - uv1.x;
    const dv2 = uv3.y - uv1.y;
    const r = du1 * dv2 - dv1 * du2;
    if (Math.abs(r) < 1e-8) {
      const nLocal = new THREE.Vector3().crossVectors(e1, e2).normalize();
      const nW = nLocal.clone().applyMatrix3(nmat).normalize();
      const centerLocal = new THREE.Vector3().add(p1).add(p2).add(p3).divideScalar(3);
      const upLocal = new THREE.Vector3().subVectors(p1, centerLocal).normalize();
      const upW = upLocal.clone().applyMatrix3(nmat).normalize();
      return { normalW: nW, upW };
    }
    const tangent = new THREE.Vector3()
...
function settleMeshToVisibleFace(mesh, type, number, duration = 700) {
  const axes = getFaceTextureAxesWorldForMesh(mesh, type, number);
  if (!axes) return;
  const { normalW: nW0, upW: upW0 } = axes;
  const dieCenterW = getDieWorldCenterMeshGeneric(mesh);
  const faceOut = new THREE.Vector3()
  ...
  const deltaWorld = new THREE.Quaternion().multiplyQuaternions(delta2, delta1);
  const currentWorldQ = new THREE.Quaternion();
  mesh.getWorldQuaternion(currentWorldQ);
  const targetWorldQ = new THREE.Quaternion().multiplyQuaternions(deltaWorld, currentWorldQ);
  let targetLocalQ = targetWorldQ.clone();
  if (mesh.parent) {
    const parentWorldQ = new THREE.Quaternion();
    mesh.parent.getWorldQuaternion(parentWorldQ);
    targetLocalQ = parentWorldQ.clone().invert().multiply(targetWorldQ);
  }
  const startQ = mesh.quaternion.clone();
  const start = performance.now();
  function tick(t) {
    const e = Math.min(1, (t - start) / duration);
    const ease = 1 - Math.pow(1 - e, 3);
    const q = new THREE.Quaternion().slerpQuaternions(startQ, targetLocalQ, ease);
    mesh.setRotationFromQuaternion(q);
    if (e < 1) requestAnimationFrame(tick);
  }
  requestAnimationFrame(tick);
...
function updateBlobShadow() {
  ...
  if (die.userData?.floatingTarget) {
...
function roll(type, options = true) {
  ...
  const targetClient = opts.targetClient || opts.targetScreen || null;
  const persistAfterLanding = opts.persist === true || opts.hold === true;
  const targetPoint = targetClient
    ? screenToRestPlanePoint(targetClient.x, targetClient.y)
    : null;
  const targetX = targetPoint ? targetPoint.x : 0;
  const targetY = targetPoint ? targetPoint.y : restY;
  const targetZ = targetPoint ? targetPoint.z : 0;
  ...
  const keepExistingDice = persistAfterLanding && Boolean(targetPoint);
  if (!keepExistingDice && die) scene.remove(die);
  const rollMesh = createDie(type, null, { preserveExisting: keepExistingDice, hideShadow: Boolean(targetPoint) });
  die = rollMesh;
  rollMesh.userData.floatingTarget = Boolean(targetPoint);
  rollMesh.userData.persistAfterLanding = keepExistingDice;
  if (keepExistingDice) heldDice.push(rollMesh);
  if (blobShadow) blobShadow.visible = !targetPoint;

  const startY = cinematic ? targetY + (targetPoint ? 2.4 : 6.3) : targetY + 1.8;
  const startZ = cinematic ? (targetPoint ? targetZ : 0.4) : (rng() - 0.5) * 0.6;
  const startX = cinematic ? targetX : (rng() - 0.5) * 0.6;
  rollMesh.position.set(startX, startY, startZ);

  const baseScale = cinematic ? DIE_CINE_SCALE : DIE_NORMAL_SCALE;
  rollMesh.scale.set(baseScale, baseScale, baseScale);

  rollMesh.rotation.set(
    rng() * Math.PI,
    rng() * Math.PI,
    rng() * Math.PI
  );
  ...
  function animate(time) {
    ...
    rollMesh.position.y = THREE.MathUtils.lerp(startY, targetY, yNorm);

    if (cinematic) {
      rollMesh.position.z = THREE.MathUtils.lerp(startZ, targetZ, easeOutCubic(t));
      rollMesh.position.x = THREE.MathUtils.lerp(startX, targetX, t);
    } else {
      rollMesh.position.x *= 1 - 0.02;
      rollMesh.position.z *= 1 - 0.02;
    }

    const spinEase = 1 - t * (cinematic ? 0.9 : 0.85);
    rollMesh.rotation.x += (spinX * 0.016) * spinEase;
    rollMesh.rotation.y += (spinY * 0.016) * spinEase;
    rollMesh.rotation.z += (spinZ * 0.016) * spinEase;

    renderer.render(scene, camera);
    if (t < 1) requestAnimationFrame(animate);
    else {
      // We now prefer a pre-chosen targetValue (for seeded RTC rolls).
      const value = targetValue || getFacingNumberForMesh(rollMesh, type);

      if (targetValue) {
        try {
          // Snap this specific die to the chosen face so concurrent/persistent
          // lobby initiative dice do not steal each other's final orientation.
          settleMeshToVisibleFace(rollMesh, type, value, 600);
        } catch (e) {
          console.warn('[Dice3D] settleMeshToVisibleFace failed', e);
        }
      }
...
function openBagOverlay() {
  ...
  buttons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // 🔹 Let the host app broadcast this roll over its own RTC/event bus.
      //     Falls back to the older window.peer hook if no callback is installed.
      try {
        const seat =
        ...
        if (typeof onRollRequestCb === 'function') onRollRequestCb(packet);
        else window.peer?.send?.(packet);
      } catch (e) {
        console.warn('[Dice3D] Failed to emit dice roll request', e);
      }
    });
  });
...
export const Dice3D = {
  ...
  init(opts = {}) {
    ...
    if (typeof opts.onRollRequest === 'function') {
      onRollRequestCb = opts.onRollRequest;
    }
    ensureScene();
    ensureResultEl();
  },
  ...
  roll(type = 'd20', arg = true) {
    ...
    if (typeof arg === 'boolean') {
      ...
      targetClient = arg.targetClient || arg.targetScreen || null;
      persist = arg.persist === true || arg.hold === true;
    }
    ...
    if (String(type).toLowerCase() === 'd100') {
      ...
      roll(type, { cinematic, rng, targetValue: value, targetClient, persist });
    }
  ...
  hide(options = {}) {
    ...
    bagEl = null;
    clearBagShowcase();
    if (options?.clear !== false) clearActiveDice();
  },
