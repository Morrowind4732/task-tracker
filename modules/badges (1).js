...
function _rebuildBuffRows(el, info){
  ...
  for (const b of buffs){
    ...
    // FILTER LOGIC:
    //
    // We ONLY want true stat buffs/debuffs here,
    // like "+3/+3 EOT" or "-2/-0 until EOT".
    //
    // We do NOT want keyword/ability grants like
    // "Flying EOT", "Lifelink until EOT", etc.,
    // because those should already be represented
    // on the right badge panel as abilities.
    //
    // Heuristic:
    //   - keep if it starts with "+" or "-"
    //     OR it contains a digit
    //   - otherwise skip it
    // -------------------------------------------------
    const startsPlusMinus = raw.startsWith('+') || raw.startsWith('-');
    const hasNumber       = /\d/.test(raw);

    const keep = startsPlusMinus || hasNumber;
    if (!keep) {
      // e.g. "Flying EOT" → drop from buff bubble
      continue;
    }
...
function isCreatureLike(info, el){
  ...
  for (const keyword of creatureishKeywords){
    if (allTypesLower.includes(keyword)) return true;
  }

  if (info?.__rulesPT) {
    const pf = info.__rulesPT.powFinal;
    const tf = info.__rulesPT.touFinal;
    if (Number.isFinite(pf) && Number.isFinite(tf)) {
      if (pf > 0 || tf > 0) return true;
    }
  }

  const gp = info?.__grant?._pow;
  const gt = info?.__grant?._tou;
  if (Number.isFinite(gp) && Number.isFinite(gt)) {
    if (gp > 0 || gt > 0) return true;
  }
...
function followLoop(info){
  ...
  const tick = () => {
    ...
    // cache sticker scale for buffWrap
